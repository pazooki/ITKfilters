...
*/
    ...
    typedef ForwardFFTImageFilter <
      InputImageType> FFTFilterType;
    typename FFTFilterType::Pointer fftFilter = FFTFilterType::New();
    fftFilter->SetInput( this->GetInput() );
    fftFilter->Update();
    fftFilter->UpdateLargestPossibleRegion();
    ...
    ones->SetRegions( this->GetInput()->GetRequestedRegion() );
    ones->SetOrigin( this->GetInput()->GetOrigin() );
    ones->SetSpacing( this->GetInput()->GetSpacing() );
    // ones->SetRegions( fftFilter->GetOutput()->GetRequestedRegion() );
    // ones->SetOrigin( fftFilter->GetOutput()->GetOrigin() );
    // ones->SetSpacing( fftFilter->GetOutput()->GetSpacing() );
    ones->Allocate();
    ones->FillBuffer( 1 );

    typename OutputImageType::SizeType size = this->GetInput()->GetLargestPossibleRegion().GetSize();
    std::cout << "Ones size: " << size << std::endl;
    ...
    shifter->SetInverse( false );
    shifter->Update();
    shifter->UpdateLargestPossibleRegion();

    std::cout << "Multiplier" << std::endl;
    std::cout << "fftFilter size: " << fftFilter->GetOutput()->GetLargestPossibleRegion().GetSize() << std::endl;
    ...
    realImage->Update();
    realImage->UpdateLargestPossibleRegion();
    ...
    imaginaryImage->UpdateLargestPossibleRegion();
    ...
    magnitude->Update();
    magnitude->UpdateLargestPossibleRegion();

    std::cout << "inverse FFT" << std::endl;
    typedef InverseFFTImageFilter <
      ComplexImageType> InverseFFTFilterType;
